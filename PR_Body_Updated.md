...
### 9. Log Persistence Failure (Zero Logs Written)
- **What happened**: Subagent logs were not being written to the `latest.log` file, especially for short-lived tasks.
- **Why**: The `activityLogger` had a `bufferLimit` of 50, meaning logs were held in memory and never flushed if the session ended before reaching 50 entries. Furthermore, there was no `process.on('exit')` hook to flush remaining logs upon termination.
- **How fixed**: Reduced the `bufferLimit` to 1 for immediate flushing and implemented a `process.on('exit')` hook inside the `ActivityLogger` singleton to ensure all buffered logs are preserved when the process terminates.

### 10. API Capacity/429 Error Subagent Crashes
- **What happened**: The subagent would immediately crash and halt when encountering an API-level 429 (Too Many Requests) or Capacity Error (e.g., model overloaded), even with the auto-router enabled.
- **Why**: The `chat.sendMessageStream` call inside the subagent's `local-executor` lacked an internal retry loop. When a capacity error was thrown mid-flight or at initiation, it bypassed the tool-level circuit breaker and terminated the entire subagent loop.
- **How fixed**: Implemented a robust exponential backoff retry loop directly around the `callModel` execution. The subagent now intelligently waits and retries up to 10 times upon encountering `RetryableQuotaError` or string-matching 429/capacity errors, while also emitting progress updates to the UI so the user knows it's retrying.
