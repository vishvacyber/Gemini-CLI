...
| `enableUserInput`         | `boolean`  | `false`        | Allow user input on the browser window during automation (non-headless only).   |
...
You can re-enable input by setting `enableUserInput` to `true`.
