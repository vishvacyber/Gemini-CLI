...
describe('reportError', () => {
  ...
  const expectedMeta = {
    generator: '@google/gemini-cli',
    type: 'error-report',
    description:
      'Gemini CLI error diagnostic report. This file is safe and was generated by the Gemini CLI tool for debugging purposes.',
  };
  ...
  it('should log fallback message if writing report fails', async () => {
    ...
    // Mock writeFile to simulate a write failure (mkdir now auto-creates dirs)
  ...
  it('should output _meta as the first key in the JSON', async () => {
    const error = new Error('Meta order test');
    await reportError(error, 'Order test.', undefined, 'general', testDir);

    const expectedReportPath = getExpectedReportPath('general');
    const reportContent = await fs.readFile(expectedReportPath, 'utf-8');
    const keys = Object.keys(JSON.parse(reportContent));
    expect(keys[0]).toBe('_meta');
  });

  it('should create the reporting directory if it does not exist', async () => {
    const error = new Error('Dir creation test');
    const nestedDir = path.join(testDir, 'nested', 'error-reports');

    await reportError(error, 'Dir test.', undefined, 'general', nestedDir);

    const expectedReportPath = path.join(
      nestedDir,
      `gemini-client-error-general-${MOCK_TIMESTAMP}.json`,
    );
    const reportContent = await fs.readFile(expectedReportPath, 'utf-8');
    const parsedReport = JSON.parse(reportContent);
    expect(parsedReport._meta).toEqual(expectedMeta);
  });
});
