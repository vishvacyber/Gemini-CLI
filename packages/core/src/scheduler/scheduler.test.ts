...
describe('Scheduler (Orchestrator)', () => {
  ...
  describe('Tool Hallucination Repair', () => {
    it('should repair kebab-case to snake_case via normalization', async () => {
      mockToolRegistry.getTool.mockImplementation((name) => {
        if (name === 'read_file') return mockTool;
        return undefined;
      });
      mockToolRegistry.getAllToolNames.mockReturnValue(['read_file']);
      ...
      expect(mockStateManager.enqueue).toHaveBeenCalledWith([
        expect.objectContaining({
          request: expect.objectContaining({
            name: 'read_file',
            originalRequestName: 'read-file',
          }),
        }),
      ]);
    });

    it('should repair hallucinated name via fuzzy matching (distance <= 2)', async () => {
      mockToolRegistry.getTool.mockImplementation((name) => {
        if (name === 'read_file') return mockTool;
        return undefined;
      });
      ...
      const requests: ToolCallRequestInfo[] = [
        {
          callId: 'call-1',
          name: 'rd_file', // Distance 2 from read_file
          args: {},
          isClientInitiated: false,
          prompt_id: 'p1',
        },
      ...
      expect(mockStateManager.enqueue).toHaveBeenCalledWith([
        expect.objectContaining({
          request: expect.objectContaining({
            name: 'read_file',
            originalRequestName: 'rd_file',
          }),
        }),
      ]);
    });

    it('should NOT repair if match is ambiguous', async () => {
      mockToolRegistry.getTool.mockReturnValue(undefined);
      mockToolRegistry.getAllToolNames.mockReturnValue(['tool_1', 'tool_2']);
      ...
      const requests: ToolCallRequestInfo[] = [
        {
          callId: 'call-1',
          name: 'tool', // Distance 2 from both tool_1 and tool_2
          args: {},
          isClientInitiated: false,
          prompt_id: 'p1',
        },
      ...
      // Should remain errored (tool not found)
      expect(mockStateManager.enqueue).toHaveBeenCalledWith([
        expect.objectContaining({
          status: CoreToolCallStatus.Error,
          request: expect.objectContaining({
    ...
    it('should repair by combining normalization and fuzzy matching', async () => {
      mockToolRegistry.getTool.mockImplementation((name) => {
        if (name === 'list_directory') return mockTool;
        return undefined;
      });
      mockToolRegistry.getAllToolNames.mockReturnValue(['list_directory']);
      ...
      const requests: ToolCallRequestInfo[] = [
        {
          callId: 'call-1',
          name: 'lst-directory', // Normalized: lst_directory. Distance 1 from list_directory.
          args: {},
          isClientInitiated: false,
          prompt_id: 'p1',
        },
      ...
      expect(mockStateManager.enqueue).toHaveBeenCalledWith([
        expect.objectContaining({
          request: expect.objectContaining({
            name: 'list_directory',
            originalRequestName: 'lst-directory',
          }),
        }),
      ]);
    });

    it('should be case-insensitive during repair', async () => {
      mockToolRegistry.getTool.mockImplementation((name) => {
        if (name === 'read_file') return mockTool;
        return undefined;
      });
      ...
      const requests: ToolCallRequestInfo[] = [
        {
          callId: 'call-1',
          name: 'READ-FILE', // Normalized: READ_FILE. Exact match (case-insensitive) for read_file.
          args: {},
          isClientInitiated: false,
          prompt_id: 'p1',
        },
