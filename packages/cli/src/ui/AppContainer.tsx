...
export const AppContainer = (props: AppContainerProps) => {
  ...
  useEffect(() => {
    if (
      !(settings.merged.ui.showBanner === false || config.getScreenReader()) &&
      bannerVisible &&
      bannerText
    ) {
...
----------------------------------------------------------------
  ...
  useEffect(() => {
    // Respect window title visibility settings.
    if (!settings.merged.ui.showWindowTitle) return;
    ...
    settings.merged.ui.showWindowTitle,
    config,
    stdout,
  ]);
