...
describe('Settings Loading and Merging', () => {
  ...
  describe('migrateDeprecatedSettings', () => {
    ...
    // Regression test for the tools.* multi-migration overwrite bug:
    // without consolidation, the approvalMode setValue and the LLMCorrection
    // setValue race and one wins, dropping the other.
    it('should preserve both tools.approvalMode and tools.disableLLMCorrection migrations together', () => {
      const userSettingsContent = {
        tools: { approvalMode: 'plan', disableLLMCorrection: true },
      };
      (fs.readFileSync as Mock).mockImplementation(
        (p: fs.PathOrFileDescriptor) => {
          if (normalizePath(p) === normalizePath(USER_SETTINGS_PATH))
