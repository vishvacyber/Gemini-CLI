...
- **How fixed**: Implemented a 5-second timeout for the PowerShell command and provided an empty map fallback to ensure the CLI remains responsive.
...
- **How fixed**: Implemented log externalization to \~/.gemini/logs/latest.log\ with a 500-line rolling buffer, drastically reducing memory footprint. Log flushing is now synchronous to ensure reliability (Fixes Gemini bot's review).
...
- **Validation**:
  ...
  - Confirmed slash commands work reliably with various line endings.
